  ...
  if (!userId) {
    return (
      <article className="rounded-[1.7rem] border border-white/10 bg-[#111111]/76 p-4 backdrop-blur-3xl">
        <h2 className="text-lg font-semibold text-[#E5E7EB]">התראות</h2>
        <p className="mt-1 text-xs text-[#E5E7EB]/55">לאחר התחברות יוצגו כאן עדכוני מצב על הנכסים שלך</p>
      </article>
    );
  }
  ...
  return (
    <article className="rounded-[1.7rem] border border-white/10 bg-[#111111]/76 p-4 backdrop-blur-3xl">
      <div className="mb-2 flex items-center gap-2">
        <Bell size={16} className="text-[#E5E7EB]/75" />
        <h2 className="text-lg font-semibold text-[#E5E7EB]">התראות</h2>
      </div>
      {loading && <p className="text-xs text-[#E5E7EB]/55">טוען התראות...</p>}
      <div className="space-y-2">
        {items.map((item) => (
          <div key={item.id} className="flex items-center gap-2 rounded-xl border border-white/10 bg-black/30 px-3 py-2">
            {item.type === 'threat' ? (
              <ShieldAlert size={14} className="text-[#FF3B30]" />
            ) : item.type === 'success' ? (
              <ShieldCheck size={14} className="text-[#007AFF]" />
            ) : (
              <Bell size={14} className="text-[#E5E7EB]/65" />
            )}
            <div className="min-w-0 flex-1">
              <p className="truncate text-xs font-semibold text-[#E5E7EB]/90">{item.title}</p>
              <p className="text-[10px] text-[#E5E7EB]/52">{item.time}</p>
            </div>
          </div>
        ))}
      </div>
      {!loading && items.length === 0 && <p className="text-xs text-[#E5E7EB]/50">אין התראות פעילות כרגע</p>}
    </article>
  );
}
